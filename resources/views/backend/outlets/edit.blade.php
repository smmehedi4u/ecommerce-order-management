...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Outlet</h2>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <form action="{{ route('outlets.update', $outlet) }}" method="POST"
                    ...
                    <div style="margin-bottom: 15px;">
                        ...
                        <input name="name" value="{{ old('name', $outlet->name) }}" required
                            style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 4px;">
                    </div>

                    <div style="margin-bottom: 15px;">
                        <label for="location" style="display: block; margin-bottom: 5px;">Location</label>
                        <input name="location" type="text" value="{{ old('location', $outlet->location) }}" required
                            style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 4px;">
                    </div>
