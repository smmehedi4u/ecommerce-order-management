...
<x-app-layout>
    <x-slot name="header">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <h2 style="font-weight: 600; font-size: 1.25rem; color: #1f2937; line-height: 1.5;">All Users</h2>
            <a href="{{ route('users.create') }}" style="text-decoration: none; padding: 6px 12px; background-color: #4f46e5; color: white; border-radius: 4px;">
                Create New User
            </a>
        </div>
    </x-slot>
                    ...
                    <tbody>
                        @foreach ($users as $user)
                            <tr style="border-bottom: 1px solid #ddd;">
                                ...
                                <td style="padding: 10px; border: 1px solid #ccc;">{{ ucfirst(str_replace('_', ' ', $user->role)) }}</td>
                                <td style="padding: 10px; border: 1px solid #ccc;">
                                    <a href="{{ route('users.edit', $user) }}" style="margin-right: 10px; background-color: #05bd2d; color: #ffffff; padding: 5px 10px; border-radius: 4px; cursor: pointer; text-decoration: none;">Edit</a>
